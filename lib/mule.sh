...
install_mule() {
  local install_dir=${1}
  local cache_dir=${2}
  echo -n "-----> Installing Mule...$cache_dir"
  let start=$(nowms)
  ...
  if [ ! -d $cache_dir/mule-standalone-3.9.0 ] ; then
    ...
    chmod +x $cache_dir/mule-standalone-3.9.0/bin/mule
    chmod +w $cache_dir/mule-standalone-3.9.0/apps/
    chmod +w $cache_dir/mule-standalone-3.9.0/
